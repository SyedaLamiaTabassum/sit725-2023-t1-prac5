...
let url = 'http://localhost:3000/api/fandom';
let fandom = {paht:'',title:''}
...
describe('test POST api', function(){
    it('post fandom to DB', function(done){
        request.post({url:url,form:fandom}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
...
describe('test DELETE api', function(){
    it('delete a fandom', function(done){
        request.delete({url:url,form:fandom}, function(a,b,c){
            //todo asserts here
            done();
        });
    });
